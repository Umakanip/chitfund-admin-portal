...
try {
    ...
    // Get unallocated schedules directly from database (customer_id IS NULL)
    $unallocatedStmt = $conn->prepare("SELECT id, month_number, status, customer_id FROM chit_schedules 
                                      WHERE scheme_id = ? AND customer_id IS NULL
                                      ORDER BY month_number ASC");
    $unallocatedStmt->execute([$schemeId]);
    $unallocatedSchedules = $unallocatedStmt->fetchAll();
    
    // Get total schedules count for error message
    $totalStmt = $conn->prepare("SELECT COUNT(*) FROM chit_schedules WHERE scheme_id = ?");
    $totalStmt->execute([$schemeId]);
    $totalSchedules = $totalStmt->fetchColumn();
    ...
    if (empty($unallocatedSchedules)) {
        $conn->rollBack();
        $allocatedCount = $totalSchedules;
        sendResponse(false, null, "No unallocated months found. All {$allocatedCount} month(s) are already allocated.", 400);
    }
    
    // Distribute members across unallocated months
    $memberCount = count($members);
    $scheduleCount = count($unallocatedSchedules);
    $allocated = 0;
    
    // Calculate how many times each member should be allocated
    // If more schedules than members, some members get multiple months
    $allocationsPerMember = floor($scheduleCount / $memberCount);
    $remainingSchedules = $scheduleCount % $memberCount;
    ...
    foreach ($unallocatedSchedules as $schedule) {
        ...
        if ($scheme['chit_type'] === 'fixed') {
            // For fixed chit, amount is predetermined (total amount)
    ...
    sendResponse(true, [
        'message' => "Allocated {$allocated} month(s) to {$memberCount} member(s)",
        'summary' => $summary
